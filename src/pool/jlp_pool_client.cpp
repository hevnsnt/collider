...
JLPPoolClient::JLPPoolClient()
    ...
    , port_(17403)
    , timeout_ms_(100)  // 100ms recv timeout - releases SSL mutex quickly for sender
...
bool JLPPoolClient::connect(const std::string& host, uint16_t port) {
    ...
    // Small delay to let connection stabilize before starting threads
    // This prevents race conditions on fast systems
...
bool JLPPoolClient::send_hello() {
    // AUTH payload: worker_name[64] + password[32] = 96 bytes
    JLPAuthPayload auth;
    memset(&auth, 0, sizeof(auth));
    strncpy(auth.worker_name, worker_name_.c_str(), sizeof(auth.worker_name) - 1);
    // password_ is set via authenticate() - leave empty if no password
    strncpy(auth.password, password_.c_str(), sizeof(auth.password) - 1);

    // Use AUTH message type (0x01) to match server protocol
    return send_message(JLPMessageType::AUTH, &auth, sizeof(auth));
}

bool JLPPoolClient::request_work(WorkAssignment& work) {
    // Clear previous work so we don't return stale data
    {
        std::lock_guard<std::mutex> lock(work_mutex_);
        current_work_ = WorkAssignment{};
    }

    // Small delay to ensure auth is fully processed before requesting work
    std::this_thread::sleep_for(std::chrono::milliseconds(100));

    // Send work request to get assignment
    ...
    while (running_) {
        ...
        if (std::chrono::duration_cast<std::chrono::milliseconds>(now - start).count() > 30000) {  // 30s request timeout
            break;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
...
bool JLPPoolClient::send_message(JLPMessageType type, const void* data, size_t size) {
    ...
    // Header format: [MAGIC:4][TYPE:1][FLAGS:1][LENGTH:2] = 8 bytes
    JLPHeader header;
    header.magic[0] = 'K';
    header.magic[1] = 'A';
    header.magic[2] = 'N';
...
bool JLPPoolClient::receive_message(JLPHeader& header, std::vector<uint8_t>& payload) {
    ...
    if (use_tls_ && ssl_) {
        ...
        if (received != sizeof(header)) {
            ...
            // SSL_ERROR_SYSCALL with timeout errno = socket timeout, not disconnect
            if (ssl_err == SSL_ERROR_SYSCALL) {
#ifdef _WIN32
                int sys_err = WSAGetLastError();
                if (sys_err == WSAETIMEDOUT || sys_err == WSAEWOULDBLOCK || sys_err == 0) {
                    last_receive_was_timeout_ = true;
                    return false;
                }
#else
            ...
            // SSL_ERROR_ZERO_RETURN = clean shutdown, also treat as timeout to retry
...
void JLPPoolClient::receiver_loop() {
    while (running_ && connected_) {
        ...
        if (receive_message(header, payload)) {
            ...
            // Timeout - yield to sender thread before retrying
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
            continue;
        } else if (connected_ && auto_reconnect_) {
            // Actual connection loss - reconnect WITHOUT starting new threads
            // (we're already inside receiver_loop, sender_loop is still running)
#ifdef COLLIDER_HAS_OPENSSL
            if (use_tls_) cleanup_tls();
#endif
            closesocket(socket_);
            socket_ = INVALID_SOCK;
            connected_ = false;
            ...
            while (running_ && auto_reconnect_) {
                ...
                // Reconnect: create new socket + TLS, but do NOT start new threads
                        ...
                        if (::connect(socket_, result->ai_addr, (int)result->ai_addrlen) != SOCK_ERROR) {
                            ...
                            if (use_tls_) {
                                if (init_tls()) {
                                    connected_ = true;
                                    reconnected = true;
                                    std::cout << "[Pool] TLS connection established" << std::endl;
                                } else {
                                    closesocket(socket_);
                                    socket_ = INVALID_SOCK;
                                }
    ...
    while (running_) {
        {
            ...
            while (!dp_queue_.empty() && batch.size() < 100) {
                ...
                jlp_dp.type = dp.type;
                jlp_dp.dp_bits = static_cast<uint8_t>(dp.dp_bits);
...
void JLPPoolClient::handle_server_message(const JLPHeader& header,
    ...
    // Convert raw type byte to enum for switch
    ...
    switch (msg_type) {
        ...
        case JLPMessageType::WORK_ASN: {
            // Debug output only when enabled
            if (debug_mode_) {
                std::cerr << "[DEBUG] WORK_ASN payload size: " << payload.size()
                          << " (expected: " << sizeof(JLPWorkAssignment) << ")" << std::endl;
            ...
            if (payload.size() >= sizeof(JLPWorkAssignment)) {
                const JLPWorkAssignment* work =
                    reinterpret_cast<const JLPWorkAssignment*>(payload.data());
                ...
                std::lock_guard<std::mutex> lock(work_mutex_);
                memcpy(current_work_.public_key, work->public_key, 33);
                memcpy(current_work_.range_start, work->range_start, 32);
                memcpy(current_work_.range_end, work->range_end, 32);
                current_work_.dp_bits = work->dp_bits;
                current_work_.work_id = work->puzzle_id;  // Use puzzle_id as work_id

                std::cout << "[Pool] Received work assignment (Puzzle: " << work->puzzle_id
                          << ", DP bits: " << (int)work->dp_bits << ")" << std::endl;
        ...
        case JLPMessageType::DP_ACK: {
            // DP acknowledged by server
            break;
        }

        case JLPMessageType::STATS_RSP: {
            // Parse status update (32 bytes)
            if (payload.size() >= 32) {
                std::lock_guard<std::mutex> lock(stats_mutex_);
        ...
        case JLPMessageType::PONG: {
            // Keepalive response
            break;
        }

        case JLPMessageType::MSG_ERROR: {
            std::string error(payload.begin(), payload.end());
            // Suppress "Invalid DP" noise - just count them silently
            if (error.find("Invalid DP") != std::string::npos) {
                std::lock_guard<std::mutex> lock(stats_mutex_);
                stats_.rejected_dps++;
            } else {
        ...
        default:
            if (debug_mode_) {
                std::cerr << "[Pool] Unknown message type: 0x"
